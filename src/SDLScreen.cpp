...
bool SDLScreen::initialize(int w, int h, int s, bool debug_panels)
{
    width = w;
    height = h;
    scale = s;
    show_debug_panels = debug_panels;
    ...
    constexpr int pattern_w = 256;
    constexpr int pattern_h = 128;

    constexpr int nametable_w = 256;
    constexpr int nametable_h = 240;

    int window_w = screen_w_px;
    int window_h = screen_h_px;
    ...
    if (show_debug_panels) {
        ...
        constexpr float sidebar_panel_w = 512.0f;
        ...
        window_w = static_cast<int>(screen_w_px + gap + sidebar_panel_w);
...
{
    if (!show_debug_panels || !palette_texture) {
        return;
    }
...
{
    if (!show_debug_panels || !pattern_texture) {
        return;
    }
...
{
    if (!show_debug_panels || !nametable_texture) {
        return;
    }
